...
/**
 * Get all blood requests with optional search/filters (Donor - to see available requests)
 * GET /api/v1/requests?page=1&limit=10&bloodGroup=O+&location=city&urgency=CRITICAL
 */
export const getAllBloodRequests = async (
  ...
  try {
    ...
    if (filters?.bloodGroup && filters.bloodGroup.trim()) {
      params.bloodGroup = filters.bloodGroup.trim();
    }
    if (filters?.location && filters.location.trim()) {
      params.location = filters.location.trim();
    }
    if (filters?.urgency && filters.urgency.trim()) {
      params.urgency = filters.urgency.trim();
    }

    console.log('🌐 getAllBloodRequests - Token:', token ? '✅ Present' : '❌ Missing');
    console.log('🌐 getAllBloodRequests - API Call Params:', params);

    const response = await axiosInstance.get(
      API.BLOOD_REQUESTS.GET_ALL,
      buildAuthConfig(token, params)
    );
    
    console.log('🌐 getAllBloodRequests - API Response:', response.data);
    
    return response.data;
  } catch (error: any) {
    console.error('🌐 getAllBloodRequests - API Error:', error);
    throw new Error(error.response?.data?.message
      || error.message || 'Failed to fetch blood requests');
  }
}
 ...
 */
export const getMyAcceptedBloodRequests = async (page: number = 1, limit: number = 10, token?: string) => {
  try {
    const response = await axiosInstance.get(
      API.BLOOD_REQUESTS.GET_MY_ACCEPTED,
